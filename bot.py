...
async def is_tx_processed(wallet_id: int, tx_hash: str) -> bool:
    ...
    result = await db_pool.execute_one(
...
async def get_wallets():
    return await db_pool.execute("SELECT * FROM wallets")
...
async def update_last_tx(wallet_id: int, tx: str):
    await db_pool.execute_write(
        "UPDATE wallets SET last_tx=? WHERE id=?", 
        (tx, wallet_id)
    )

async def validate_api_keys():
    """Validate that required API keys are present for monitored networks"""
    wallets = await get_wallets()
    networks_used = set(w[2] for w in wallets)
    
    evm_needed = any(n in ETHERSCAN_NETWORKS for n in networks_used)
    solana_needed = "solana" in networks_used
    
    if evm_needed and not ETHERSCAN_API_KEY:
        logger.error("❌ EVM wallets detected but ETHERSCAN_API_KEY not set!")
    
    if solana_needed and not SOLSCAN_API_KEY:
        logger.error("❌ Solana wallets detected but SOLSCAN_API_KEY not set!")
...
def validate_address(address: str, net_key: str) -> Tuple[bool, str]:
    """Return (is_valid, error_message)"""
    address = address.strip()
    
    if net_key == "tron":
        if not address.startswith("T"):
            return False, "TRON addresses must start with 'T'"
        if len(address) != 34:
            return False, "TRON addresses must be exactly 34 characters"
        return True, ""
    elif net_key == "ton":
        if len(address) < 48:
            return False, "TON addresses must be at least 48 characters"
        return True, ""
    elif net_key == "solana":
        if not (32 <= len(address) <= 44):
            return False, "Solana addresses must be 32-44 characters"
        valid_chars = set("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz")
        if not all(c in valid_chars for c in address):
            return False, "Solana addresses must be valid base58"
        return True, ""
    elif net_key in ETHERSCAN_NETWORKS:
        if not address.startswith("0x"):
            return False, "EVM addresses must start with '0x'"
        if len(address) != 42:
            return False, "EVM addresses must be exactly 42 characters"
        return True, ""
    
    return False, f"Unknown network: {net_key}"
            ...
            async with session.get(url, timeout=timeout, headers=headers or {}) as r:
                if r.status == 200:
                    try:
                        ...
                        # Try to decode as text and parse JSON
                        text = await r.text()
                        try:
                            return json.loads(text)
                        except json.JSONDecodeError:
                            logger.error(f"Invalid JSON response from {url[:100]}")
                            return None
                elif r.status == 429:
                    ...
                    logger.warning(f"HTTP {r.status} for {url[:100]}")
                    return None
...
async def check_solana(address: str, last_tx: str) -> List[Tuple[str, str, float]]:
    ...
    if isinstance(js, dict):
        if "success" in js and not js["success"]:
            logger.error(f"Solscan API error: {js.get('message', 'Unknown error')}")
            return []
        txs = js.get("data", [])
    ...
    for tx in txs:
        ...
        for t in token_transfers:
            ...
            if dest == address.lower():
                amt = float(t.get("amount", 0))
                sym = t.get("symbol", "SOL")
                
                if amt > 0:
                    new_txs.append((sig, sym, amt))
        
        # For SOL transfers, we need to parse the transaction details
        # This is a simplified check - you may want to enhance this
        if not token_transfers and tx.get("status") == 1:
            # Check if it's an incoming SOL transfer
            # This requires more complex parsing of the transaction
            pass
    ...
    while True:
        try:
            ...
            if wallets:
                ...
                async def check_with_limit(wallet):
                    async with semaphore:
                        return await check_single_wallet(wallet)
                
                tasks = [check_with_limit(w) for w in wallets]
...
async def cmd_start(m: Message):
    ...
    await m.answer(
        ...
        "/info — Bot info & links\n\n"
        f"⚡ **Inline Mode:**\n"
        f"Type @{bot_me.username} {{label}} in ANY chat\n"
        f"to quickly paste wallet addresses!"
    )
...
async def cb_network_selected(c: CallbackQuery, state: FSMContext):
    ...
    is_valid, error_msg = validate_address(address, net_key)
    if not is_valid:
        display = ALL_NETWORKS.get(net_key, net_key)
...
async def fsm_get_label(m: Message, state: FSMContext):
    ...
    if not label or label.isspace():
        await m.answer("❌ Label cannot be empty or just spaces.")
        return
    ...
    try:
        ...
        status_text += (
            ...
            f"⚡ **Quick Access:**\n"
            f"Type @{bot_me.username} {label} in any chat to paste this address!"
        )
...
async def cmd_list_wallets(m: Message):
    ...
    bot_me = await bot.me()
    text += f"\n⚡ Type @{bot_me.username} {{label}} in any chat to quickly paste addresses!"
...
async def main():
    ...
    logger.info(f" EVM (unified etherscan): {len(ETHERSCAN_NETWORKS)} chains")
    ...
    # Import and setup wallet groups if available
    try:
        ...
        logger.info("✅ Wallet groups integration loaded")
    except ImportError:
        logger.info("ℹ️ Wallet groups module not found - skipping")
